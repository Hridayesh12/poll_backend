...
//   const excel_file = excelmaker(giorno, dio, uuid, r)
//   setTimeout(() => {
...
router.post("/excel", async (req, res) => {
    ...
    var r = false;
    var uuid = dio.lobbyId;
    const filename = await excelmaker(giorno, dio, uuid, r)
    console.log('name',filename);
    console.log(typeof(filename));
    if(fs.existsSync(`./excel/${filename}.xlsx`))
    {
        console.log('it works')
            res.download(`./excel/${filename}.xlsx`,filename, { dotfiles: 'deny' }, function (err) {
        if (err) {
          return err;
        }
      });
...
router.get("/download/:stuid/:name", function (req, res) {
  ...
  if (fs.existsSync(`${f}.xlsx`)) {
    res.download(`${f}.xlsx`, { dotfiles: "deny" }, function (err) {
      if (err) {
        return err;
      }
...
router.post("/getMarks", async (req, res) => {
  console.log("it works");
  Voters.find(
    { "Subject.SubjectValue": req.body.subject },
    {
      Subject: { $elemMatch: { SubjectValue: req.body.subject } },
      mail: 1,
      name: 1,
    }
  ).then((saveditem) => {
...
const excelmaker = async (giorno, dio, uuid, r, leng) => {
  let l = giorno.length;
  var lname = uuid;
  var date = lname.substr(0, lname.indexOf("t"));
  const subexist = lname.includes("s");
  ...
  if (subexist) {
   ...
   f = dio.lobbyName.substr(0, 12) + date + subject;
  var diego = dio.studentformId.length;
  let workbook = new Excel.Workbook();
  for (j = 0; j < l; j++) {
    var gio = giorno[j];
  ...
  for (var w = 0; w < diego; w++) {
    ...
    await workbook.xlsx.writeFile(`./excel/${f}.xlsx`);
    console.log("participants added");
  }
  if (r) {
    const zip = new AdmZip();
